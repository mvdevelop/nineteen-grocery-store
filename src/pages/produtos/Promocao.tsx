...
import type { ProdutoPromo } from "@/types/products";
...
export default function Promocao() {
  const [produtos] = useState<ProdutoPromo[]>(PromosJSON as ProdutoPromo[]);
  ...
  const handleAddToCart = (produto: ProdutoPromo) => {
    dispatch(
      addToCart({
        id: produto.id,
        nome: produto.nome,
  ...
  const calcularDesconto = (precoOriginal: number, precoPromo: number) => {
              ...
              return (
                <SwiperSlide key={produto.id}>
                  <div className="group bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border-2 border-transparent hover:border-amber-300 relative h-full flex flex-col">
                    ...
                    {/* Barra de progresso (estoque limitado) */}
                    {produto.estoque && (
