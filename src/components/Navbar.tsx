    ...
    const { data: listener } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (event === "SIGNED_IN" && userData && !welcomeShown) {
          const name = (userData as { user_metadata?: { name?: string } }).user_metadata?.name ??
              ...
              {user ? (
                <div className="flex items-center space-x-3">
                  <span className="text-sm font-medium">
                    Olá, {(user.user_metadata as { name?: string })?.name ?? (user as { email?: string }).email}
                  </span>
                  <button
              ...
              {user ? (
                <div className="pt-4 border-t">
                  <p className="mb-3 font-medium">
                    Olá, <span className="font-bold">{(user.user_metadata as { name?: string })?.name ?? (user as { email?: string }).email}</span>
                  </p>
                  <button
                    onClick={() => {
                      handleLogoutClick();
